...
export async function getDynamicKeys() {
  try {
    ...
    if (settings?.apiKeys) {
      const raw = JSON.parse(settings.apiKeys) as Record<string, string>;
      // Decrypt each key before returning — values are AES-256-GCM encrypted at rest
      const decrypted: Record<string, string> = {};
      for (const [k, v] of Object.entries(raw)) {
        decrypted[k] = v ? decryptApiKey(v) : '';
      }
      return decrypted;
    }
  } catch (error) {
